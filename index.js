...
console.log(color('3'))
console.clear()
console.log(color('=> Fenzo77.js Di Pantau Oleh Kang Bakso'))
console.log(color('=> Fenzo77.js Di Pantau Oleh Kang Bakso'))
console.log(color('=> Fenzo77.js Di Pantau Oleh Kang Bakso'))
CFonts.say(`Fax Online`, {
            font: 'tiny',
            align: 'left',
            gradient: ['red', 'magenta']
        })
console.log(color('=> Source code version:', 'yellow'), color('Fardan v1.1', 'cyan'))
console.log(color('=> Bug? Error? Suggestion? Visit here:', 'yellow'), color('6281523940672'))
console.log(color(`${sessionName}.json`), color(`BOT is now online!`, 'red'))
console.log(color('[DEV]', 'cyan'), color('Welcome back, Owner! Hope you are doing well~', 'magenta'))
console.log(color('F', 'green'), color('a', 'red'), color('r', 'yellow'), color('d', 'purple'), color('a', 'blue'),  color('n', 'green'), color('BOT', 'cyan'))
                      console.log(color('SUPPORT by htttps://youtube.com/FaxBFaraxx354', 'cyan'))
    const hisoka = hisokaConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Fax Faraxx Multi Device','Safari','1.0.0'],
        auth: state
    })
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./faraxx")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    hisoka.sendContact(num, global.owner, )
                    teks = `Hai Kak @${num.split("@")[0]} Selamat Datang Di Group *${metadata.subject}* \nCongrats!! 🎉`
                    hisoka.sendMessage(num, {text: teks, mentions:[num]})
                } else if (anu.action == 'remove') {
                    teks = `Yah Kak nya Out Dari ${metadata.subject}\n Happy Trus Yah Kak *Congrats!!* 🎉`
                    hisoka.sendMessage(num, {text: teks, mentions:[num]})
